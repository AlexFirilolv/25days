      ...
      .then((data: Memory[]) => {
        if (Array.isArray(data)) {
          ...
          if (memoryForDay && memoryForDay.blocks.length > 0) {
            setBlocks(memoryForDay.blocks.map(b => ({
              ...b, 
              id: Math.random().toString(36).substr(2, 9),
              formatting: b.formatting || {}
            })));
          } else {
            setBlocks([{ id: 'initial-title', block_type: 'title', content: '', formatting: { fontFamily: 'Arial, sans-serif' } }]);
          }
        }
      });
  }, [day]);

  const handleBlockChange = (id: string, content: string, formatting?: TextFormatting) => {

    setBlocks(blocks.map(b => b.id === id ? { ...b, content, formatting } : b));
  };

  const handleBlockTypeChange = (id: string, block_type: BlockType) => {
    setBlocks(blocks.map(b => b.id === id ? { ...b, block_type, content: '', formatting: { fontFamily: 'Arial, sans-serif' } } : b));
  };

  const addBlock = () => {
    setBlocks([...blocks, { id: Math.random().toString(36).substr(2, 9), block_type: 'paragraph', content: '', formatting: { fontFamily: 'Arial, sans-serif' } }]);
  };
                                            ...
                                            <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                                ...
                                                {block.block_type === 'image' ? (
                                                    <ImageEditor
                                                        ...
                                                        onFileChange={(e) => handleFileChange(block.id, e)}
                                                    />
                                                ) : (
                                                    <RichTextEditor
                                                        content={block.content}
                                                        formatting={block.formatting || {}}
                                                        onChange={(content, formatting) => handleBlockChange(block.id, content, formatting)}
                                                        placeholder={`Enter your ${block.block_type}...`}
                                                        rows={block.block_type === 'title' ? 2 : 4}
                                                    />
                                                )}
                                            </div>
                                        )}
