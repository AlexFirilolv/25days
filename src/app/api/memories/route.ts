    ...
    if (blocks) {
      ...
      for (const block of blocks) {
        await connection.execute(
          'INSERT INTO memory_blocks (memory_id, block_type, content, formatting, sort_order) VALUES (?, ?, ?, ?, ?)',
          [memoryId, block.block_type, block.content, JSON.stringify(block.formatting || {}), block.sort_order]
        );
      }
    }
...
export async function GET() {
  try {
    ...
    for (const memory of memories) {
        ...
        // Parse formatting JSON for each block
        memory.blocks = blocks.map((block: any) => {
          let formatting = {};
          try {
            if (block.formatting) {
              // Check if it's already an object or a string
              if (typeof block.formatting === 'string') {
                formatting = JSON.parse(block.formatting);
              } else {
                formatting = block.formatting;
              }
            }
          } catch (error) {
            console.error('Error parsing formatting JSON:', error, 'Raw formatting:', block.formatting);
            formatting = {};
          }
          return {
            ...block,
